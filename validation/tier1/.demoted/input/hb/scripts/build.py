...
Raw input (gitignored, fetched on demand):
  zhu2018/hb_genbank.gb   GenBank records MG772099-MG772439 (341 seqs:
                  alphaA/alphaD/betaA globin, ~3-16 population isolates per
                  species per gene).
...
Output (pipeline-shaped, tips = binomials, "." / " " -> "_"):
  ...
  tree.nwk         species tree: Fig S1 topology (Johansson 2013 + Li 2016),
                   ...
                   contrast selection assumes a time tree (see _fixture_lib.date_tree)
...
from _fixture_lib import date_tree, write_fasta  # noqa: E402

RAW_DIR = HERE / "zhu2018"
RAW_GB = RAW_DIR / "hb_genbank.gb"
...
# Fig S1 topology, read directly off the published figure (Zhu et al. 2018,
# SI Appendix pnas.201720487si.pdf, page 2 -- caption cites Johansson et al.
# 2013 MPE 69:852 and Li et al. 2016 MPE 104:14 as its sources, but this is
# transcribed node-by-node from Zhu's own rendered tree, not re-derived from
# either source paper). Aegithalidae is the outgroup to all of Paridae;
# within Paridae, (Periparus+Pardaliparus) is sister to (Poecile+Lophophanes),
# and that pair is sister to a third clade containing Sylviparus, Cyanistes,
# Pseudopodoces and the four Parus/Machlolophus species -- with Sylviparus
# nested as the most basal member of THAT clade, not basal to all of Paridae.
# (An earlier version of this constant had Sylviparus basal to all of
# Paridae instead, and Periparus+Pardaliparus sister to the OTHER two clades
# combined rather than to Poecile+Lophophanes specifically -- confirmed
# wrong against the actual figure, not just a stylistic rearrangement.)
# Branch lengths are added by IQ-TREE below.
_TOPOLOGY = (
    ...
    "(Sylviparus_modestus,(Cyanistes_cyanus,(Pseudopodoces_humilis,"
...
def _assert_landmarks(aln: dict[str, dict[str, str]]) -> None:
    ...
    # Sylviparus modestus (low-altitude; nested one level inside its own
    # clade per the corrected _TOPOLOGY above, not basal to all of Paridae)
    # ALSO shows Thr34 - one candidate explanation is retention of the
    # ancestral avian state (chicken alphaA is Thr34 too; AncParidae is the
    # derived Ala34) via a single reversal in the Sylviparus lineage, which
    # costs the same under either topology since Sylviparus is still one
    # terminal branch either way. It is a genuine background species
...
def _iqtree(concat: Path, per_gene: dict[str, Path], outdir: Path) -> None:
    ...
    date_tree(outdir, set(_OUTGROUP))
...
def main() -> None:
    ...
    for g in _GENES:
        write_fasta(aln_dir / f"{g}.fasta", aln[g])
