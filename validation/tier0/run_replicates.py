...
exec bash "{repo}/run_phenotype_single.sh" {cls} {trait} {arch_args}
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--min-divergent-fraction", default="0.5")
    ap.add_argument("--seed0", type=int, default=71000)
    ...
    for arch, setname, tkey in itertools.product(archetypes, sets, tree_keys):
        ...
        for k in range(a.n_replicates):
            ...
            run_sh.write_text(_RUN_SH.format(
                ...
                cls=cls, trait=TRAITNAME, arch_args=arch_args,
            ))
            run_sh.chmod(0o755)
