---
title: "WebGestalt ORA analysis for CAAS gene lists"
author: "Miguel Ramon"
date: "`r Sys.Date()`"
citations:  
  output: 
...
# WebGestalt ORA analysis for CAAS gene lists
...
ora_result_dir <- file.path(resultsDir, "2.CAAS/6.CAAS_WEBGESTALT/")
...
# Load the gene_lists
geneList_dir <- file.path(resultsDir, "2.CAAS/5.AntiCAAS_filtout/malignant_prevalence/true_caas")
geneList_files <- list.files(path = geneList_dir, pattern = "\\.genes$", full.names = TRUE, recursive = TRUE)
new_geneList_mp <- lapply(geneList_files, read_table, col_names = FALSE)
names(new_geneList_mp) <- geneList_files %>%
  gsub("^.*/([^/]+)/([^/]+).genes$", "\\1_\\2", .) %>%
  paste0(., "_", "malignant_prevalence")

geneList_dir <- file.path(resultsDir, "2.CAAS/5.AntiCAAS_filtout/neoplasia_prevalence/true_caas")
geneList_files <- list.files(path = geneList_dir, pattern = "\\.genes$", full.names = TRUE, recursive = TRUE)
