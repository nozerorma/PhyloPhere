...
mp_rand_files <- list.files(
  path = file.path(rand_dir, "malignant_prevalence_tb"), 
...
np_rand_files <- list.files(
  path = file.path(rand_dir, "neoplasia_prevalence_tb"), 
  pattern = "_aggregated_results.csv", 
  full.names = TRUE, 
  recursive = TRUE
)
...
output_dir <- file.path(resultsDir, "2.CAAS/4.5.Rand_results/tb")
createDir(output_dir)

# Malignant prevalence
for (approach in names(mp_aggregated_files$total)) {
  path <- file.path(output_dir, "malignant_prevalence_tb")
  createDir(path)
...
for (approach in names(np_aggregated_files$total)) {
  path <- file.path(output_dir, "neoplasia_prevalence_tb")
  createDir(path)
  output_file <- file.path(path, paste0(approach, "_aggregated_results.csv"))
...
gene_list_dir <- file.path(resultsDir, "2.CAAS/4.5.Rand_results/tb/GeneLists")
...
# Output directory for gene lists
gene_list_dir <- file.path(resultsDir, "2.CAAS/4.5.Rand_results/tb/GeneLists.01")
createDir(gene_list_dir)
