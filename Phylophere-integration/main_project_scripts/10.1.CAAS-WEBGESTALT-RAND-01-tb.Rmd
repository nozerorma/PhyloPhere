...
# Initialize the result directory
ora_result_dir <- file.path(resultsDir, "2.CAAS/7.RAND_WEBGESTALT.01/tb/total")
createDir(ora_result_dir)

# Load the gene_lists
geneList_dir <- file.path(resultsDir, "2.CAAS/4.5.Rand_results/tb/GeneLists.01/malignant_prevalence/total")
...
geneList_dir <- file.path(resultsDir, "2.CAAS/4.5.Rand_results/tb/GeneLists.01/neoplasia_prevalence/total")
geneList_files <- list.files(path = geneList_dir, pattern = "\\.txt$", full.names = TRUE, recursive = TRUE)
...
# Load the gene_lists
geneList_dir <- file.path(resultsDir, "2.CAAS/4.5.Rand_results/tb/GeneLists.01/malignant_prevalence/pattern1")
geneList_files <- list.files(path = geneList_dir, pattern = "\\.txt$", full.names = TRUE, recursive = TRUE)
new_geneList_mp <- lapply(geneList_files, read_table, col_names = FALSE)
names(new_geneList_mp) <- geneList_files %>%
  gsub("^.*/([^/]+)/([^/]+).txt$", "\\1_\\2", .) %>%
...
geneList_dir <- file.path(resultsDir, "2.CAAS/4.5.Rand_results/tb/GeneLists.01/neoplasia_prevalence/pattern1")
geneList_files <- list.files(path = geneList_dir, pattern = "\\.txt$", full.names = TRUE, recursive = TRUE)
new_geneList_np <- lapply(geneList_files, read_table, col_names=FALSE)
names(new_geneList_np) <- geneList_files %>%
  gsub("^.*/([^/]+)/([^/]+).txt$", "\\1_\\2", .) %>%
