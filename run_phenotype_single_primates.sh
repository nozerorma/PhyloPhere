...
WORK_BASE="/data/samanthafs/scratch/lab_anavarro/mramon/3.Work_dirs/final"
...
SOURCE_RUN_SUBDIR="${SOURCE_RUN_SUBDIR:-20260318_201152}"
...
if [ "$IS_TOY" = true ]; then
    ...
    TAG=""
    ALI_DIR="/data/samanthafs/scratch/lab_anavarro/mramon/4.Generate_alignments_from_codons/alignments/Primates_BMGE/PROT"
    CYCLES="1000000"
...
if [ "$RUN_VEP" = true ]; then
    for d in "$CDS_DIR" "$TRACK_DIR"; do
        if [ ! -d "$d" ]; then
            echo "Error: VEP directory not found: $d"
            exit 1
        fi
    done
fi
