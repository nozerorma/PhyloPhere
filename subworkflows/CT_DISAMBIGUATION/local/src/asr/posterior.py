            ...
            if ":" in site_data:
                ...
                for idx, token in enumerate(posterior_tokens):
                    ...
                    if not token or "(" not in token or ")" not in token:
                        logger.debug(
                            f"Skipping malformed token '{token}' at site {site_num} position {idx}"
                        )
                        continue
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Failed to parse token '{token}' at site {site_num}: {e}"
                        )
                        continue
            else:
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        if node_id_map:
            try:
                f.write(
                    json.dumps(
                        {"__metadata__": {"node_id_map": node_id_map}},
                        ensure_ascii=False,
                    )
                    + "\n"
                )
                ...
                logger.debug(
                    "Could not serialize node_id_map to JSONL metadata; skipping metadata line."
                )
        ...
        for node_id in sorted(posteriors.keys()):
            try:
                ...
                logger.debug(
                    f"Failed to serialize node {node_id} posteriors to JSONL: {e}"
                )
