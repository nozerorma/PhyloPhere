...
from .tree_parser import TreeNode, parse_newick, get_mrca, find_node_by_name, get_tip_labels
    ...
    **REDESIGNED FOR DYNAMIC PAIRS**: This function now accepts a variable number of 
    ...
    for idx, group in enumerate(focal_pairs, 1):
        ...
        if not present:
            ...
            logger.info(f"Found {len(present)}/{len(group)} species for focal pair {idx}")
    ...
    logger.info(f"Found {len(contrast_present)}/{len(contrast_species)} total contrast species")
    ...
    if len(valid_focal_ids) >= 2:
        ...
        if len(focal_nodes_objs) >= 2:
            ...
            if mrca_contrast and mrca_contrast.node_id is not None:
                ...
                logger.debug(f"MRCA contrast (all {len(valid_focal_ids)} focal nodes): node {mrca_contrast.node_id}")
            else:
                logger.warning("Could not find MRCA of contrast species")
        else:
            logger.warning(f"Could not find focal node objects for MRCA computation")
...
def identify_convergence_nodes_from_file(
    ...
    if root.node_id is None:
        logger.warning("Tree nodes have no IDs - use build_node_mapping with RST file for PAML trees")
    
    return identify_convergence_nodes(
        root,
        focal_pairs,
        contrast_species,
        require_all
