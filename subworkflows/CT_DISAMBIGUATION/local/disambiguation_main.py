#!/usr/bin/env python3
"""
CAAS Aggregation Main Coordinator

Processes all genes found in CAAS metadata file, leveraging existing
single_gene_pipeline logic to produce consolidated CSV outputs with
...
def parse_arguments():
    ...
    parser.add_argument("--tree", required=True,
                        help="Phylogenetic tree file (Newick/Nexus)")
    parser.add_argument("--caas-metadata", required=True,
                        help="CAAS metadata file (all genes)")
    parser.add_argument("--trait-file", required=True,
                        help="Trait file with species/contrast/trait/pair columns")
    parser.add_argument("--output-dir", required=True,
                        help="Output directory for results")
    parser.add_argument("--ensembl-genes-file", default=None,
                        help="TSV/CSV with gene column; used to strictly match alignment prefixes")

    # ASR configuration
    parser.add_argument("--asr-mode", choices=["compute", "precomputed"],
                        default="precomputed",
                        help="ASR mode (default: precomputed)")
    parser.add_argument("--asr-model", default="lg",
                        help="Substitution model for ASR (default: lg)")
    parser.add_argument("--asr-cache-dir", default=None,
                        help="Directory for precomputed ASR files")
    parser.add_argument("--posterior-threshold", type=float, default=0.0,
                        help="Posterior probability threshold (default: 0.0)")

    # Analysis configuration
    parser.add_argument("--convergence-mode", choices=["focal_clade", "mrca"],
                        default="mrca",
                        help="Convergence analysis mode (default: mrca)")
    parser.add_argument("--taxid-mapping", default=None,
                        help="TaxID to species mapping file")
    ...
    parser.add_argument("--codeml-concurrency", type=int, default=None,
    ...
    parser.add_argument("--skip-gene-lists", action="store_true",
...
def main():
    ...
    try:
        ...
        unique_genes = metadata_df['GenePos'].apply(
    ...
    logger.info(f"  Conserved-pair ASR flags carried in master CSV rows")
    ...
    try:
        ...
        if export_info:
            ...
            logger.info(f"  Skipping CSV generation; files produced by DB exporter: {caas_files}")
            ...
            try:
                if caas_results:
                    ...
                    logger.info(f"  Per-gene JSONs: {len(gene_jsons)} files in {json_dir}")
            except Exception as e:
        ...
        if not args.skip_gene_lists:
            try:
                ...
                if caas_results:
                    ...
                    try:
                        import csv as _csv
                        master_csv = caas_files[0] if caas_files else (output_dir / "caas_convergence_master.csv")
                        if Path(master_csv).exists():
                            with open(master_csv, 'r', encoding='utf-8') as _f:
                                reader = _csv.DictReader(_f)
                                for row in reader:
                                    # Normalize common boolean-like fields
                                    for bool_field in ('is_significant', 'is_stable', 'isAntiCAAS', 'ASR_ConsAntiCAAS'):
                                        if bool_field in row:
                                            val = row[bool_field]
                                            if isinstance(val, str):
                                                v = val.strip().lower()
                                                row[bool_field] = v in ('true', '1', 'yes', 'y')
                                            else:
                        ...
                        logger.warning(f"Failed to read master CSV for gene list export: {e}")
                ...
                if source_results:
                    ...
                    all_pattern_count = len(gene_list_paths.get("all", {}).get("by_pattern", {}))
                    sig_pattern_count = len(gene_list_paths.get("significant", {}).get("by_pattern", {}))
                    all_change_count = len(gene_list_paths.get("all", {}).get("by_change", {}))
                    sig_change_count = len(gene_list_paths.get("significant", {}).get("by_change", {}))
                    logger.info(
        ...
        try:
            master_csv_raw = caas_files[0] if caas_files else (output_dir / "caas_convergence_master.csv")
