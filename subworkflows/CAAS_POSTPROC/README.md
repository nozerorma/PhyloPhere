...
The CAAS Post-Processing workflow consists of two main components:
...
└── local/
    ...
    └── 5.2.CAAS_postproc.Rmd             # R markdown reporting skeleton
```
...
```
{outdir}/caas_postproc/filter_exploratory/
├── minlen2_maxcaas60/
...
Output structure:
```
{outdir}/caas_postproc/filter_selected/
...
Reports generated in `{outdir}/caas_postproc/reports/`:
...
Default configuration in `conf/caas_postproc.config`:
...
params {
    ...
    postproc_outdir = "${params.outdir}/caas_postproc"
...
R markdown report template: 5.2.CAAS_postproc.Rmd (original manuscript analysis)
...
- Configuration examples: `conf/caas_postproc.config`
