...
def detect_extreme_genes(discovery_df, gene_length_df, percentile=0.99, trait_col="Trait"):
    ...
    GS0 exclusion: gs0 is treated as null hypothesis and excluded from removal criteria.
    ...
    # Exclude gs0 from removal criteria (null hypothesis)
    if has_caap_group:
        discovery_for_removal = discovery_df[discovery_df['CAAP_Group'] != 'gs0'].copy()
        print(f"CAAP mode detected: calculating extreme gene thresholds per group (excluding gs0)", file=sys.stderr)
    else:
        discovery_for_removal = discovery_df.copy()
    
    # Merge gene lengths (use discovery_for_removal which excludes gs0)
    ...
    # Count unique CAAS positions per gene per trait (and group if CAAP mode, excluding gs0)
    ...
    # Calculate threshold per trait (and per group if CAAP mode, only for non-gs0 groups)
    thresholds = (
        gene_summary
...
def detect_dubious_genes(discovery_df, gene_length_df, cluster_file, iqr_multiplier=3.0, trait_col="Trait"):
    ...
    if has_caap_group:
        discovery_for_removal = discovery_df[discovery_df['CAAP_Group'] != 'gs0'].copy()
        print(f"CAAP mode detected: calculating IQR thresholds per group (excluding gs0)", file=sys.stderr)
    else:
        discovery_for_removal = discovery_df.copy()
    ...
    # Calculate per-trait (and per-group if CAAP, only for non-gs0) IQR thresholds
    ...
    # If CAAP mode, only consider clusters within the same non-gs0 group
    if has_caap_group and 'CAAP_Group' in cluster_df.columns:
        # Exclude gs0 from cluster consideration
        cluster_df_filtered = cluster_df[cluster_df['CAAP_Group'] != 'gs0'].copy()
        # Build set of (Gene, CAAP_Group) tuples with clusters (excluding gs0)
        genes_with_clusters = set(
            cluster_df_filtered[cluster_df_filtered['ClusteringFlag'] == 'Discarded']
            [['Gene', 'CAAP_Group']]
...
def apply_gene_filter(discovery_df, removed_genes_df, mode='both'):
    ...
    print(f"Removed {n_removed} CAAS positions from {len(gene_list)} genes (mode: {mode})", file=sys.stderr)
    print(f"Remaining: {n_after}/{n_before} positions ({100*n_after/n_before:.1f}%)", file=sys.stderr)
...
"""
    ...
    print(f"Positions retained: {len(filtered_df)}/{len(discovery_df)} ({100*len(filtered_df)/len(discovery_df):.2f}%)", file=sys.stderr)
