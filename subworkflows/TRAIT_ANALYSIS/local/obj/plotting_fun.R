...
species_plot_profile <- function(n_species, n_taxa = n_species, n_rings = 0L) {
  ...
  list(
    ...
    tree = list(
      ...
      fruit_gap = clamp_value(0.08 + 0.0008 * n_species + 0.01 * pmax(0, n_rings - 2), 0.08, 0.18),
      secondary_offset = clamp_value(0.08 + 0.0008 * n_species + 0.01 * pmax(0, n_rings - 2), 0.08, 0.18),
      taxa_bar_pwidth = clamp_value(0.10 - 0.0003 * n_species, 0.05, 0.10),
      taxa_bar_offset = clamp_value(0.10 + 0.0012 * n_species + 0.01 * pmax(0, n_rings - 2), 0.10, 0.22),
      axis_text_size = clamp_value(8.0 - 0.020 * n_species, 4.0, 8.0),
      ...
      n_label_nudge = clamp_value(7.5 + 0.5 * pmax(0, n_rings - 2) - 0.045 * n_species, 2.5, 8.6),
      asterisk_size = clamp_value(7.0 - 0.025 * n_species, 2.5, 7.0),
      asterisk_nudge = clamp_value(6.0 + 0.45 * pmax(0, n_rings - 2) - 0.035 * n_species, 2.0, 7.2),
      asterisk_y = clamp_value(0.15 - 0.0010 * n_species, 0.05, 0.15),
