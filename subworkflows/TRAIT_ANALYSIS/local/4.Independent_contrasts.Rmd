...
# Attach tip metadata via tidytree::as.treedata (as in 2.Phenotype_exploration.Rmd)
# rather than the %<+% + geom_tippoint/geom_tiplab route, which calls ggtree
# internals (geom_segment2 -> ggplot2:::is.waive) that ggplot2 4.x no longer has.
...
selected_pairs_tree <- ggtree(contrast_tree_data, aes(color = contrast_label),
                              layout = "fan", open.angle = 15, size = 0.4) +
  geom_text2(aes(label = species), hjust = -0.1, size = 2.5) +
  scale_color_manual(
    name = "Contrast group",
...
ggsave(
  ...
  selected_pairs_tree, device = "png", width = 15, height = 15, dpi = "retina"
)
