...
asr_tree.f <- function(df, tree, trait, taxon_of_interest) {
  ...
  cont_obj <- phytools::setMap(cont_obj, colors=colorRampPalette(c("white",tableau_10["vermillion"]))(100))
  ...
  for(i in seq_len(Ntip_rescaled)){
    ...
    if(tip_df$immediate_parent[i]){
      ...
      bg_val <- ifelse(tip_df$direction[i]=="up", tableau_10["vermillion"], tableau_10["bluish_green"])
    ...
    if(tip_df$cumulative_derived[i]){
      ape::tiplabels(pch=22, bg=tableau_10["green"], tip=tip_pos, cex=2, offset=62)
    }
  }
