...
```{r phenoplot, fig.height=10, fig.width=15, message=TRUE, warning=TRUE, dev='png'}
# Objects
...
```{r violin_plot, fig.height=10, fig.width=7, message=TRUE, warning=TRUE, dev='png'}
### PLOT FUNCTION --------------------------------------------------------------
...
```{r asr_plot, fig.height=20, fig.width=17, message=TRUE, warning=TRUE, dev='png'}
# 4. ASR PLOTS  ------------------------------------------------------
...
if (!plot_ready) {
  ...
  asr_plot_path <- file.path(phylo_distribution_dir, paste0(trait, "_asr_tree.png"))
  tryCatch(
    {
      grDevices::png(
...
# This section is problematic. We have three optional colums: N, branch_trait, and second_trait.
...
if(isTRUE(has.N)){
  tree_plot_step3 <- tree_plot_step2 +
    geom_text(aes(label = N_sum), nudge_x = 6.1, fontface = "bold", size = 6, vjust = 0.5)
} else {
  tree_plot_step3 <- tree_plot_step2
}
...
# Final tree plot with title and subtitle
