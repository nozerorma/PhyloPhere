...
or copy the files from `fa_transvar_ref/`. The workflow can also be pointed at
an explicit TransVar config file via `TRANSVAR_CFG` or `--vep_transvar_cfg`
when the binary is installed in a separate environment.
