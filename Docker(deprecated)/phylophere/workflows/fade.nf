...
include { FADE_REPORT       } from "${baseDir}/subworkflows/FADE/fade_report.nf"
...
def ali_tuples_from_dir(String ali_dir, String direction, Set wanted) {
    ...
    def tuples = all_files.collect { f ->
        ...
        (!wanted || wanted.contains(gid)) ? [gid, direction, f] : null
    }.findAll { it != null }
...
workflow FADE {
    ...
    main:
        ...
        fade_report_top    = FADE_REPORT(Channel.value('top'),    top_jsons   )
        fade_report_bottom = FADE_REPORT(Channel.value('bottom'), bottom_jsons)
