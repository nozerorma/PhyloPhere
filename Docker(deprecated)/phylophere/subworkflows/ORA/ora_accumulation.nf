...
/*
#  ORA (WebGestalt) accumulation-randomization process wrapper
#  Consumes gene_lists produced by CT_ACCUMULATION_RANDOMIZE
#  (published under accumulation/randomization/gene_lists/).
*/

process ORA_ACCUMULATION_REPORT {
    tag "ora_accumulation"
    label 'process_reporting'

    publishDir path: "${params.outdir}/accumulation/ora", mode: 'copy', overwrite: true, pattern: '{ORA_files/**,ora_results/**,ora_summary/**,ora_plots/**}'
    ...
    def outdir             = "${params.outdir}/accumulation/ora"
    def project_name       = params.ora_accumulation_project_name ?: 'ORA_Accumulation'
    ...
    def enable_spider      = params.ora_enable_spider_plot          ? 'TRUE' : 'FALSE'
    ...
    def compare_metric     = params.ora_compare_metric     ?: 'jaccard'
    ...
    if (params.use_singularity || params.use_apptainer) {
        ...
        /usr/local/bin/_entrypoint.sh Rscript -e "
            rmarkdown::render(
                ...
                output_file = 'ORA_accumulation.html'
            )
        "
        """
    } else {
