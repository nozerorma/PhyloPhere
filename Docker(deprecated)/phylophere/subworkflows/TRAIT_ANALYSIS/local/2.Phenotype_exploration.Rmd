...
contrast_plot.f <- function(df, palette_values = NULL) {
  ...
  p <- p +
    ...
    ggplot2::theme(
      ...
      plot.caption = ggplot2::element_text(size = 12, hjust = 0.5, margin = ggplot2::margin(t = 20)),
...
# Compute dynamic dimensions for final tree plot (keeps ring height invariant across clade sizes)
n_tips <- length(circular_tree$tip.label)
dyn_height <- max(15, round(3 + n_tips * 0.12, 1))
# Count geom_fruit rings added upstream: primary + optional secondary + taxa bar
n_fruit_rings <- 1L + isTRUE(has.secondary) + 1L
# Width: extra room for rings, clade labels and legend
dyn_width <- max(dyn_height + 5, round(dyn_height + 3 + n_fruit_rings * 2 + n_tips * 0.06, 1))
# Clade label offsets must clear all geom_fruit rings; scale with n_tips and ring count
cladelab_offset_phylopic <- round(max(14, n_tips * 0.12 + n_fruit_rings * 2.5), 1)
cladelab_offset_text     <- round(cladelab_offset_phylopic - 3, 1)
