...
class TreeNode:
    ...
    def __init__(self, name: Optional[str] = None, node_id: Optional[int] = None):
        self.name = (
            name  # Tip label (e.g., 'taxid_speciesname') or None for internal nodes
        )
...
def build_node_mapping(
    tree_file: Optional[Path] = None, rst_file: Optional[Path] = None
    ...
    logger.debug(
        f"Expected PAML node range: {min_node} to {max_node} ({expected_internal} nodes)"
    )
    ...
    logger.info("Built tree from RST labeled tree:")
    ...
    if internal_count != expected_internal:
        logger.warning(
            f"Expected {expected_internal} internal nodes, got {internal_count}"
        )
