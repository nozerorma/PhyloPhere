...
process CONCAT_DISCOVERY {
    ...
    discovery_outputs=\$(find . -type f -name "*.output" | sort)
    
    # Check if we have any files
    if [ -z "\$discovery_outputs" ]; then
        echo "Gene\tPosition\tAncestral\tDerived\tType\tForeground\tBackground" > discovery.tab
        exit 0
    fi
    
    # Save header from first file
    first_file=\$(echo "\$discovery_outputs" | head -n 1)
    cat "\$first_file" > discovery.tab
    
    # Append remaining files without headers
    echo "\$discovery_outputs" | tail -n +2 | while read -r file; do
        tail -n +2 "\$file" >> discovery.tab
    done
    """
}
...
process CONCAT_RESAMPLE {
    ...
    # Append remaining files without headers
    echo "\$resample_files" | tail -n +2 | while read -r file; do
...
process CONCAT_BOOTSTRAP {
    ...
    # Append remaining files without headers
    echo "\$bootstrap_outputs" | tail -n +2 | while read -r file; do
