...
process CONCAT_DISCOVERY {
    ...
    mapfile -t discovery_files < <(find discovery_outputs -type f -name "*.output" | sort)
...
process CONCAT_RESAMPLE {
    ...
    echo "Working directory: \$(pwd)"
    echo "Resample dir input: ${resample_dir}"
    echo "Contents of current directory:"
    ls -laR
    echo ""
...
process CONCAT_BOOTSTRAP {
    ...
    # Find all .output files and sort them
    mapfile -t bootstrap_files < <(find bootstrap_outputs -type f -name "*.output" | sort)
