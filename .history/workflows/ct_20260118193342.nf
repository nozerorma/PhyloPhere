...
workflow CT {
    if (params.ct_tool) {
        ...
        if (toolsToRun.contains('bootstrap')) {
            ...
            // Convert resample_out to path if it's a string (from params.resample_out parameter)
            def resample_path = resample_out instanceof String ? file(resample_out) : resample_out
