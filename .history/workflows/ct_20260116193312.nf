...
workflow CT {
    if (params.ct_tool) {
        ...
        if (toolsToRun.contains('discovery')) {
            ...
            // Concatenate all discovery outputs (extract files from tuples)
            CONCAT_DISCOVERY(discovery_out.map { id, file -> file }.collect())
        }
        if (toolsToRun.contains('resample')) {
            ...
            // Concatenate all resample outputs
        ...
        if (toolsToRun.contains('bootstrap')) {
            ...
            // Concatenate all bootstrap outputs (extract files from tuples)
            CONCAT_BOOTSTRAP(bootstrap_out.map { id, file -> file }.collect())
        }
    }
}
