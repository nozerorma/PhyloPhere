...
def iscaas(input_string, multiconfig=None, position_dict=None, max_conserved=0, trait=None):
    ...
    if max_conserved == 0 or not multiconfig or not multiconfig.paired_mode:
        ...
        print(f"Trait: {trait}")
        print(f"max_conserved: {max_conserved}")
        print(f"paired_mode: {multiconfig.paired_mode}")
        ...
        print(f"FG unique AAs: {fg}")
        print(f"BG unique AAs: {bg}")
        print(f"Overlapping AAs: {overlap_aas}")
        ...
        if not overlap_aas:
            ...
            if trait and multiconfig:
                ...
                print(f"FG species: {fg_species_all}")
                print(f"BG species: {bg_species_all}")
                print(f"Species2AA mapping: {species2aa}")
                
                # For each overlapping amino acid, ALL species with it must have paired species with same AA
                for aa in overlap_aas:
                    print(f"\nChecking overlapping AA: {aa}")
                    ...
                    print(f"  FG species with {aa}: {fg_with_aa}")
                    print(f"  BG species with {aa}: {bg_with_aa}")
                    
                    # Check FG species with this overlapping AA
                    for fg_sp in fg_with_aa:
                        fg_pair_id = multiconfig.get_pair(fg_sp)
                        print(f"    Checking FG species {fg_sp}, pair_id={fg_pair_id}")
                        if fg_pair_id:
                            ...
                            print(f"      Found paired BG species: {paired_bg_sp}")
                            
                            if paired_bg_sp:
                                # Check if the paired BG species has the same AA
                                paired_aa = species2aa.get(paired_bg_sp)
                                print(f"      Paired BG species {paired_bg_sp} has AA: {paired_aa}")
                                if paired_aa == aa:
                                    print(f"      ✓ CONSERVED PAIR DETECTED: {fg_pair_id}")
