...
pair_sel.f <- function(distance_matrix, overlap_df, traits_df, my_trait) {
  ...
  has_n <- "n" %in% colnames(traits_df)
  debug_log("pair_sel.f trait = %s, traits_df rows = %d, has_n = %s", my_trait, nrow(traits_df), has_n)

  trait_df <- traits_df %>%
    filter(trait == my_trait) %>%
    {
      if (has_n) dplyr::select(., species, value, n) else dplyr::select(., species, value)
    }
  ...
  distance_df <- as.data.frame(as.matrix(distance_matrix)) %>%
    ...
    filter(distance != 0) %>%  # Remove self-comparisons
    ...
    {
      if (has_n) {
        dplyr::rename(., n1 = n.x, n2 = n.y) %>%
          mutate(pair_n = ifelse(is.na(n1) | is.na(n2), NA_real_, n1 + n2))
      } else {
        .
      }
    }
  ...
  distance_df <- distance_df %>%
    ...
    {
      if (has_n) arrange(., distance, desc(diff), desc(pair_n)) else arrange(., distance, desc(diff))
    } %>%
    filter(!is.na(diff)) %>%
    {
      if (has_n) select(., species1, species2, distance, diff, pair_n) else select(., species1, species2, distance, diff)
    }
  ...
  if (has_n) dunn_results$pair_n <- numeric()
  ...
  while (current_dunn_index >= 1 && iteration_count < iteration_limit) {
    ...
    dunn_candidates <- apply(distance_df, 1, function(row) {
      ...
      query_diff <- row["diff"]
      query_pair_n <- if (has_n) row["pair_n"] else NA
      ...
      if (has_n) out$pair_n <- as.numeric(query_pair_n)
    ...
    dunn_result_cummulative <- rbind(dunn_result_cummulative, dunn_candidates) %>%
      {
        if (has_n) arrange(., desc(cluster), desc(Dunn_index), desc(diff), desc(pair_n)) else arrange(., desc(cluster), desc(Dunn_index), desc(diff))
      }

    # Select best candidate: highest Dunn index, then largest trait difference
    dunn_best <- dunn_candidates %>%
      {
        if (has_n) arrange(., desc(Dunn_index), desc(diff), desc(pair_n)) else arrange(., desc(Dunn_index), desc(diff))
      } %>%
      slice_head(n = 1)
