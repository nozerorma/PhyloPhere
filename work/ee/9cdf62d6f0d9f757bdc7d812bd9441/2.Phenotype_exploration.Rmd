...
source(file.path(workingDir, "fun/plotting_functions.R")) # Plotting utilities for trait analysis
...
stats_path <- file.path(resultsDir, "1.Data-exploration/1.Species_distribution/trait_stats.csv")
...
has.N <- FALSE
if (nzchar(N_trait) && N_trait %in% names(stats_df)) {
  has.N <- TRUE
  N.sym <- sym(N_trait)
}
...
ggplot2::ggsave(
  file.path(resultsDir, "1.Data-exploration/2.Extreme_plots", paste0(trait, "_contrast_plot.png")),
...
gen_ast_tree <- function(contrast_t, prim_tree, my_trait) {
  ...
  plot(
    ...
    main = paste(capitalized_trait, "-", clade_name), cex.main = 2
  )
  
  # Add time axis
  ape::axisPhylo(1)
  graphics::mtext("Branch length", side = 1, line = 3, cex = 1.5)
  
  # Add vertical color bar
  phytools::add.color.bar(
    Ntip(obj$tree), obj$cols, title = paste(capitalized_trait, "-", clade_name),
...
# Generate ASR tree for the selected trait
ast_tree <- list()
trait_names <- list(trait)
...
for (trait_name in trait_names) {
  png(
    filename = file.path(plotDir, paste0(trait_name, "-tree.png")),
    width = 18, height = 20, res = 320, bg = "white", units = "in"
  )
  ast_tree[[trait_name]] <- gen_ast_tree(trait_df, pruned_tree, trait_name)
  dev.off()
  
  gen_ast_tree(trait_df, pruned_tree, trait_name)
}

```

## 5. Extreme Value Plots
This section is pending a trait-naive port (requires long-format trait inputs for generic traits).

```{r extremes_plots, fig.height=10, fig.width=7, message=FALSE, warning=FALSE, dev='png', dpi = 320, eval=FALSE}
...
This section is pending a trait-naive port (requires trait-specific overlays).

```{r visual_distribution, fig.height = 15, fig.width = 15, dev = 'png', dpi = 320, eval=FALSE}
