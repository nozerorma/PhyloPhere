...
def slice(alignment_file, alignment_format, column_threshold, max_gaps=0.5):
    ...
    try:
        # Read the alignment (this will fail if sequences are not the same length)
        imported_alignment = AlignIO.read(alignment_file, alignment_format)
    except ValueError as e:
        # Handle the case where sequences are not of the same length
        print(f"Skipping {alignment_file}: {str(e)}")
        return z  # Return an empty slice_object instead of None
    ...
    # IMPORTING POSITIONS
    imported_positions = list(map(functools.partial(import_position, imported_alignment=imported_alignment),
                                  range(imported_alignment.get_alignment_length())))

    # FILTERING POSITIONS
    z.d = list(filter(functools.partial(filter_position, changes_threshold=column_threshold,
                                        max_gaps_ratio=max_gaps), imported_positions))
